...
class Die:
    ...
    def roll(self, num_rolls: int, seed: Optional[int] = 42) -> np.ndarray[np.integer]:
...
def oo_generate_sample(die_type_counts: Tuple[int],
               dice: Tuple[Die],
               num_draws: int,
               rolls_per_draw: int) -> np.ndarray[np.integer]:
    """
    Args:
    ...
    Returns:
        np.ndarray[np.integer]: A numpy array of draws, each draw being a tuple 
        of faces rolled.
    """
    ...
    # PUT YOUR CODE HERE. Make sure to return a numpy nd.array.
